...
opencode 会自动加载以下目录中的 skill（`SKILL.md`），下载 [SKILL.md](https://github.com/excellence-wh/home-ai/blob/main/skills/SKILL.md)到目录中即可：

- 项目内：`.opencode/skills/home-ai/SKILL.md`
- 全局：`~/.config/opencode/skills/home-ai/SKILL.md`

可用以下两条 curl 命令快速完成下载：

```bash
# 下载到项目内
mkdir -p .opencode/skills/home-ai && curl -fsSL https://raw.githubusercontent.com/excellence-wh/home-ai/main/skills/SKILL.md -o .opencode/skills/home-ai/SKILL.md

# 下载到全局
mkdir -p ~/.config/opencode/skills/home-ai && curl -fsSL https://raw.githubusercontent.com/excellence-wh/home-ai/main/skills/SKILL.md -o ~/.config/opencode/skills/home-ai/SKILL.md
```

### Claude Code

Claude Code 会从以下目录自动发现 skill（每个 skill 为 `<name>/SKILL.md`），下载 [SKILL.md](https://github.com/excellence-wh/home-ai/blob/main/skills/SKILL.md) 到对应目录即可，之后可用 `/home-ai` 斜杠命令调用：

- 项目内（随 git 共享）：`.claude/skills/home-ai/SKILL.md`
- 全局（所有项目可用）：`~/.claude/skills/home-ai/SKILL.md`

```bash
# 下载到项目内
mkdir -p .claude/skills/home-ai && curl -fsSL https://raw.githubusercontent.com/excellence-wh/home-ai/main/skills/SKILL.md -o .claude/skills/home-ai/SKILL.md
...
- 项目内：`.codex/skills/home-ai/SKILL.md`
- 全局：`~/.codex/skills/home-ai/SKILL.md`

```bash
# 下载到项目内
mkdir -p .codex/skills/home-ai && curl -fsSL https://raw.githubusercontent.com/excellence-wh/home-ai/main/skills/SKILL.md -o .codex/skills/home-ai/SKILL.md
...
1. 从仓库下载 [`skills/SKILL.md`](https://github.com/excellence-wh/home-ai/blob/main/skills/SKILL.md)。
2. 保存到该助手对应的 skills 目录下，命名为 `home-ai/SKILL.md`（即 `<skills 根目录>/home-ai/SKILL.md`）。
...
默认路径 `~/.config/home-ai/auth.json`，可用 `-p /path/to/auth.json` 覆盖。全局参数的 `-p` 放在参数前；子命令（`get`/`set`/`action`/`statistics`/`run`）的 `-p` 放在子命令后。
...
5. **获取统计数据**：
   ...
   还可能使用不带 `_v3` 的统计类型。详见 [issue #46](https://github.com/excellence-wh/home-ai/issues/46)
...
::: tip
完整的命令行参数说明请参考 [CLI 参数参考](/reference/cli-args)，skill 的完整定义见仓库内的 [`skills/SKILL.md`](https://github.com/excellence-wh/home-ai/blob/main/skills/SKILL.md)。
:::
