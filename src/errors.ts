...
const msg = (code: number, message: string) => `code: ${code}, message: ${message}`;
...
export class DeviceNotFoundError extends Error {
  constructor(did: string) {
    super(`未找到 did 为 '${did}' 的设备，请检查 did 是否正确`);
...
export class DeviceGetError extends Error {
  constructor(devName: string, name: string, code: number) {
    super(
      `获取设备 '${devName}' 的属性 '${name}' 时失败, code: ${code}, message: ${ERROR_CODE[String(code)] ?? "未知错误"}`,
    );
...
export class DeviceSetError extends Error {
  constructor(devName: string, name: string, code: number) {
    super(
      `设置设备 '${devName}' 的属性 '${name}' 时失败, code: ${code}, message: ${ERROR_CODE[String(code)] ?? "未知错误"}`,
    );
...
export class DeviceActionError extends Error {
  constructor(devName: string, name: string, code: number) {
    super(
      `执行设备 '${devName}' 的动作 '${name}' 时失败, code: ${code}, message: ${ERROR_CODE[String(code)] ?? "未知错误"}`,
    );
